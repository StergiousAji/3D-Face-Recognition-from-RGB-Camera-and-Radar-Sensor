...
train = np.concatenate((crd_resampled_gun1, crd_resampled_gun2, crd_resampled_nogun1, crd_resampled_nogun2))
train = np.abs(train).astype('float64')


test = np.concatenate((crd_resampled_gun3, crd_resampled_gun4, crd_resampled_nogun3, crd_resampled_nogun4))
test = np.abs(test).astype('float64')

train = train[:,4:,:,:]
test = test[:,4:,:,:]

gun1lab = np.ones((len(crd_resampled_gun1),1), dtype='float64')
gun2lab = np.ones((len(crd_resampled_gun2),1), dtype='float64')
nogun1lab = np.zeros((len(crd_resampled_nogun1),1), dtype='float64')
nogun2lab = np.zeros((len(crd_resampled_nogun2),1), dtype='float64')
train_label = np.concatenate((gun1lab, gun2lab, nogun1lab, nogun2lab))


gun3lab = np.ones((len(crd_resampled_gun3),1), dtype='float64')
gun4lab = np.ones((len(crd_resampled_gun4),1), dtype='float64')
nogun3lab = np.zeros((len(crd_resampled_nogun3),1), dtype='float64')
nogun4lab = np.zeros((len(crd_resampled_nogun4),1), dtype='float64')
test_label = np.concatenate((gun3lab, gun4lab, nogun3lab, nogun4lab))
...
json_soli_data = []
f = open('/home/elle/Documents/soli/data/radar.json')
json_soli_datas = json.load(f)
json_soli_data.append(json_soli_datas)
crd_data = []
crd_data.append(get_crd_data(json_soli_data,declutter=False))
...
crd_resampled_nogun2 = np.abs(crd_resampled_nogun2)

crd_resampled_nogun2 = crd_resampled_nogun2[:,4:,:,:]
labe = np.zeros((len(crd_resampled_nogun2),1), dtype='float64')

model = tf.keras.models.load_model('/home/elle/soli_logger/python/mymodel_19')

pred = model.evaluate(crd_resampled_nogun2, labe)

pred__ = model.predict(crd_resampled_nogun4[:,4:,:,:], batch_size=32)
pred = model.evaluate(test, test_label, batch_size=8)
pred_ = model.predict(crd_resampled_gun4[:,4:,:,:], batch_size=32)
pred_ = model.evaluate(np.abs(crd_resampled_gun3[:,4:,:,:]), gun3lab, batch_size=32)
pred = model.predict(test)

anotherattempt = model.evaluate(test[0:1000], test_label[0:1000])
