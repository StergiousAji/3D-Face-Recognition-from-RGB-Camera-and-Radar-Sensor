...
def config():
    ...
    config = rs.config()
    config.enable_stream(rs.stream.depth, 1280, 720, rs.format.z16, 30) #Params: stream, resolution_x, resolution_y, module, frame rate
...
def grab_frames():    
    ...
    depth_frames_list = np.zeros((frame_number,100,50))
    rgb_frames_list = np.zeros((frame_number,100,50, 3))
    ...
    try:   
       for i in range(frame_number):        
            ...
            depth_frame = cv2.resize(depth_scale*np.asanyarray(depth_frame.data).astype('float16'), (100, 50))
            color_frame = frames.get_color_frame()
            color_frame = cv2.resize(np.asanyarray(color_frame.data).astype('float16'), (100, 50))
            depth_frames_list[i] = depth_frame #the depth frames are in units of m    
            rgb_frames_list[i] = color_frame
            timestamps[i,0] = frames.timestamp
            timestamps[i,1] = time.time()
            print('Frame',i,'device time difference',frames.timestamp-time_1,'system time difference',(time.time()-time_2)*1e3)
            time_1 = frames.timestamp  
            time_2 = time.time()
            cv2.imshow('image_1',depth_frames_list[i])
            cv2.imshow('image_2',rgb_frames_list[i])
            cv2.waitKey(1)
	    
    finally:
        pipeline.stop()
        np.save('../data/depth_images.npy',depth_frames_list)
        np.save('../data/rgb_images.npy',rgb_frames_list)
